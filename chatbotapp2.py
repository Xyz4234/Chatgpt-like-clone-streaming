...
st.markdown("""
    ...
    .prompt-placeholder {
        position: absolute;
        top: -35px;  /* Adjust this value to position above the input field */
        left: 0;
        right: 0;
        padding: 10px;
        color: white;
...
with prompt_container:
    ...
    prompt = st.chat_input("Wassup write smth here")
    st.markdown('<div class="prompt-placeholder">Start writing in prompt</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

# Ensure the placeholder disappears after the conversation starts
